...
public final class ReciprocalArraySum {
    ...
    private static class ReciprocalArraySumTask extends RecursiveAction {
        ...
        protected void compute() {
            value = 0;
            if (endIndexExclusive - startIndexInclusive < seqFactor) {
    ...
    protected static double parManyTaskArraySum(final double[] input,
            final int numTasks) {
        double sum = 0;
        int len = getChunkSize(numTasks, input.length);
        ReciprocalArraySumTask[] tasks = new ReciprocalArraySumTask[len];

        for (int i = 0; i < len; i++) {
            tasks[i] = new ReciprocalArraySumTask(getChunkStartInclusive(i, len, input.length),
                    getChunkEndExclusive(i, len, input.length), input);
        }
